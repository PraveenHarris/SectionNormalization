...
def grade_match(match, verbose=False):
    ...
    if e_valid:
        ...
        if not vm:
            if verbose:
                print(
                    f".. {i_s}:{i_r} marked invalid, should be {e_sid}:{e_rid}")
            pts = 0

        if vm and (not sm or not rm):
            if verbose:
                print(
                    f".. {i_s}:{i_r} WRONG!, marked {o_sid}:{o_rid}, should be {e_sid}:{e_rid}")
            pts = -5
...
def grade_python(path_to_manifest, path_to_input, verbose=False, is_windows=False):
    ...
    grade(path_to_manifest, path_to_input, executable,
          verbose=verbose, is_windows=is_windows)
